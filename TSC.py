...
def user01():
    ...
    try:
        ...
        # 3️⃣ Fetch logged-in user details
        cursor.execute(
            "SELECT first_name, last_name, email, username FROM accounts WHERE id = %s",
            (session['user_id'],)
        )
        ...
        # 4️⃣ Safety check if user not found
        if user is None:
            session.clear()
            return redirect(url_for('login'))

        # 5️⃣ Render user dashboard
        return render_template('user01.html', user=user)

    except Exception as e:
        print("Error in user01 route:", e)
...
def queriesraised():
    ...
    cur.execute("SELECT * FROM queries WHERE user_id = %s", (session['user_id'],))
    user_queries = cur.fetchall()
    
    cur.close()
    conn.close()

    return render_template('queriesraised.html', queriesraised=queriesraised)
